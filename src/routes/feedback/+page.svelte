<script>
	...
	let feedbacks = [
		{
			...
			message: 'Jane showed excellent leadership skills during our recent project. Her communication was clear and she helped resolve conflicts effectively.',
			rating: 9,
			isAnonymous: false,
		...
		},
		{
			...
			message: 'Consider being more open to feedback and collaborative approaches to problem-solving.',
			rating: 6,
			isAnonymous: true,
	...
	const columns = [
		{ key: 'fromPerson', label: 'From', type: 'text', required: true, placeholder: 'Enter sender name' },
		{ key: 'toPerson', label: 'To', type: 'text', required: true, placeholder: 'Enter recipient name' },
		{ 
			key: 'feedbackType', 
			label: 'Type', 
			type: 'select',
		...
		{ key: 'subject', label: 'Subject', type: 'text', required: true, placeholder: 'Enter feedback subject' },
		{ key: 'message', label: 'Message', type: 'textarea', required: true, placeholder: 'Enter feedback message' },
	...
	function handleUpdate(updatedFeedback) {
		feedbacks = feedbacks.map((feedback) => (feedback.id === updatedFeedback.id ? updatedFeedback : feedback));
	}
