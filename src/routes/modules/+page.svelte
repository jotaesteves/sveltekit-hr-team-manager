<script>
	...
	function handleUpdate(updatedModule) {
		modules = modules.map((module) => (module.id === updatedModule.id ? updatedModule : module));
	}
...
<div class="space-y-6">
	...
	<div class="rounded-lg bg-white p-6 shadow-md">
		<h2 class="mb-4 text-xl font-semibold">Module Overview</h2>
	...
	<div class="rounded-lg bg-white p-6 shadow-md">
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
			{#each modules as module}
				<div
					class="rounded-lg border p-4 {module.isEnabled
				...
				>
					<div class="mb-2 flex items-start justify-between">
						...
						<span
							class="rounded px-2 py-1 text-xs {module.status === 'Active'
					...
					<p class="mb-3 text-sm text-gray-600">{module.description}</p>
					<button
						on:click={() => toggleModule(module.id)}
						class="w-full rounded px-3 py-2 text-sm {module.isEnabled
